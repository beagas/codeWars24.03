public class Main {
    public static void main(String[] args) {
        System.out.println(Solution.replace("KATINUKAS"));
        Solution.replace("KATINUKAS");
        System.out.println((Solution.removeExclamationMarks("Hello!")));
        System.out.println(Solution.collinearity(1,1,6,1));
        System.out.println(Solution.cockroachSpeed(89));
        System.out.println(Solution.paperWork(5,2));
        System.out.println(Solution.getTime("83W",61,80,"26W"));
        System.out.println(Solution.getTime1("450W", 3, 25, "950W"));
        System.out.println(Solution.hexColor(""));
        System.out.println(Solution.stepThroughWith("comment"));

    }
}

